"""
Copied from sentence_for_mlm_processing 12/22/2024
"""
...
"""
- MAGPIE entry 
- function get_tokens_for_word -> get the list of token indices that corr to 
word
- function get_tokens_for_offset

- get tokens for (alphabetic) word (index)
- get inputids with mask (for alphabetic) word

- pretokenization / postdetokenization

let's assume post detoke for now
- tokenize
- get mapping for particular words (verify they match) -> compare all 
alphabetical
- get mapping for all words
...
- Tokenized word
- need to store
- per spaced word (with any alphabetical) - the start/end char index, 
the start/end token indices
...
"""
class SentenceForMLMProcessingSimple:
    """
